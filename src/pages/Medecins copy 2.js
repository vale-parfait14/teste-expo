...
const Medecins = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editId) {
        await updateDoc(doc(db, "medecins", editId), {
          ...formData,
          photo: photoUrl || formData.photo,
          certifications: certificationsUrls.length > 0 ? certificationsUrls : formData.certifications
        });
        setEditId(null);
      } else {
        await addDoc(medecinsCollectionRef, {
          ...formData,
          photo: photoUrl,
