mais ajuste totalement  :"     <Tab.Pane eventKey="appointments" active={activeTab === "appointments"}>
                        ...
                        <Card key={appointment.id} className="mb-3 shadow-sm">
                          <Card.Body>
                            <div className="d-flex justify-content-between align-items-start">
                              <div>
                                <h5 className="mb-1">
                                  {appointment.structureInfo?.name || 'Structure non spécifiée'}
                                </h5>
                                <p className="mb-1">
                                  <FaUserMd className="me-2 text-primary" />
                                  Dr. {appointment.doctorInfo?.nom} {appointment.doctorInfo?.prenom}
                                  {appointment.doctorInfo?.specialite && (
                                    <span className="text-muted ms-2">({appointment.doctorInfo.specialite})</span>
                                  )}
                                </p>
                                <div className="d-flex align-items-center mb-2">
                                  <FaCalendarAlt className="me-2 text-primary" />
                                  <span>
                                    {appointment.day || 'Jour non spécifié'} - {appointment.date ? new Date(appointment.date).toLocaleDateString() : 'Date non spécifiée'}
                                  </span>
                                </div>
                                <div className="d-flex align-items-center">
                                  <FaClock className="me-2 text-primary" />
                                  <span>{appointment.timeSlot || 'Heure non spécifiée'}</span>
                                </div>
                              </div>
                              <Badge 
                                bg={
                          ...
                          <Card.Footer className="bg-white">
                            <div className="d-flex justify-content-between">
                              <small className="text-muted">
                                Créé le: {appointment.createdAt ? new Date(appointment.createdAt).toLocaleDateString() : 'Non spécifié'}
                              </small>
                              <div>
            ...
            </Tab.Pane> " qui est dans :" import React, { useState, useEffect } from 'react';
                ...
                try {
                  ...
                  for (const appointmentDoc of appointmentsSnapshot.docs) {
                    ...
                    if (appointmentData.doctorId) {
                      const doctorDoc = await getDoc(doc(db, 'medecins', appointmentData.doctorId));
                      if (doctorDoc.exists()) {
                        appointmentData.doctorInfo = { id: doctorDoc.id, ...doctorDoc.data() };
                      }
                    }
                    
                    // Récupérer les informations de la structure
                    if (appointmentData.structureId) {
                      const structureDoc = await getDoc(doc(db, 'structures', appointmentData.structureId));
                      if (structureDoc.exists()) {
                        appointmentData.structureInfo = { id: structureDoc.id, ...structureDoc.data() };
                      }
                    }
            ...
            export default PatientDashboard;
             " pour soit identique à  sous fond et la forme à :"
    <Modal 
    ...
    </Modal>" qui est dans :" // Ajoutez cette ligne en haut de votre fichier
